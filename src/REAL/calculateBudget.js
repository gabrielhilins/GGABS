export const calculateBudget = (formData, pricingModels, serviceNames) => {
  ...
  switch (service) {
    ...
    case "ecommerce": {
      ...
      total += produtos > pricing.factors.produtos.threshold ? (produtos - pricing.factors.produtos.threshold) * pricing.factors.produtos.pricePerExtra : 0;
      total += ["Mercado Pago", "PagSeguro", "Outro gateway"].includes(details.integracao) ? pricing.factors.paymentGateway.price : 0;
      budgetDetails.push({ desc: `Produtos extras (${produtos})`, qty: produtos, unit: pricing.factors.produtos.pricePerExtra, total: produtos > pricing.factors.produtos.threshold ? (produtos - pricing.factors.produtos.threshold) * pricing.factors.produtos.pricePerExtra : 0 });
